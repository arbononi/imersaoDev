...
function exibeCartaJogador() {
    ...
    var opcoesTexto = ""
    var maiorAtributo = ""
    var habilitaRadio = "enabled"
    if (!jogadorEscolhe)
      habilitaRadio = "disabled"
    for (var atributo in cartaJogador.atributos) {
        opcoesTexto += "<input type='radio' name='atributo' value='" + atributo + "' checked " + habilitaRadio + " >" + atributo + " " + cartaJogador.atributos[atributo] + "<br>"
      if (maiorAtributo == "") {
         maiorAtributo = atributo
       }
      else if (cartaJogador.atributos[atributo] > cartaJogador.atributos[maiorAtributo]){
         maiorAtributo = atributo
      }
    }
  ...
  if (!jogadorEscolhe) {
    escolherAtributoCartaJogador(maiorAtributo) 
  }
  else
    {
      console.log("Jogador escolhe o atributo")
    }
  jogadorEscolhe = !jogadorEscolhe
}
...
function jogar() {
  ...
  if (cartas.length == 0) {
    ...
    if (pontosJogador > pontosMaquina){
      htmlResultado = "<p class='resultado-final'>Fim de Jogo! Você Venceu!</p>"
    } else if(pontosMaquina > pontosJogador){
      htmlResultado = "<p class='resultado-final'>Fim de Jogo! Você Perdeu!</p>"
    } else { htmlResultado = "<p class='resultado-final'>Fim de Jogo! Houve um Empate!</p>"}
  }
...
function escolherAtributoCartaJogador(maiorAtributo){
    var radioAtributo = document.getElementsByName('atributo')
    for (var i = 0; i < radioAtributo.length; i++) {
        if (radioAtributo[i].value == maiorAtributo) {
            radioAtributo[i].checked = true
        }
    }
  console.log("Máquina escolheu o atributo")
}